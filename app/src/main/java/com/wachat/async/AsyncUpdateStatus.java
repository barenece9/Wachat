 ...
 */
public class AsyncUpdateStatus extends AsyncTask<Void, Void, Void> {
    ...
    public AsyncUpdateStatus(String userId, String status , InterfaceResponseCallback mCallback) {
        this.userId = userId;
        this.status = status;
    ...
    protected Void doInBackground(Void... params) {
        ...
        mListPairs.add(new BasicNameValuePair(WebContstants.status, CommonMethods.getUTFEncodedString(status)));

        try {
            HttpConnectionClass mHttpConnectionClass = new HttpConnectionClass(WebContstants.UrlUpdateStatus);
            String result = mHttpConnectionClass.getResponse(mListPairs);
            if (result != null && result.length() > 0) {
                startParsing(result);
            }
        } catch (Throwable e) {
            LogUtils.i("AsyncUnblockUser",""+e.getLocalizedMessage());
            mException = e;
        }
        return null;
    }
